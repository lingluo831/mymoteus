...
```
python -m moteus_gui.tview --target 1[,2,3]...
```
...
```
python -m moteus.moteus_tool --target 1 --zero-offset
```
...
```
python -m moteus.moteus_tool --target 1 --flash path/to/file.elf
```
...
```
python -m moteus.moteus_tool --fdcanusb /path/to/fdcanusb
```

To force a particular python-can method, you can use:

```
python -m moteus.moteus_tool --can-iface socketcan --can-chan can0
```
